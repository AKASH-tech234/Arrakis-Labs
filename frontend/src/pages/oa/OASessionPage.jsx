...
import { useOASession, useOATimer, useAutosave, useTabVisibility, useBeforeUnload } from "../../hooks/useOA";
...
export default function OASession() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const response = await oaService.runCode(sessionId, currentQuestion.refId, {
        code,
        language,
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "#0A0A08" }}>
        <Card className="px-6 py-5 max-w-md border-[#7F1D1D] bg-[#2A0F0F] text-[#FCA5A5]">
          ...
          <p style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>{sessionError}</p>
          <div className="mt-6">
            <Button onClick={() => navigate("/oa")} variant="secondary" size="md">
              Back to Dashboard
            </Button>
          </div>
        </Card>
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#0A0A08" }}>
      ...
      <div className="bg-[#121210] border-b border-[#1A1814] px-6 py-3 flex items-center justify-between shadow-lg">
        ...
        <div className="flex items-center gap-4">
          ...
          {lastSaved && !isSaving && (
            <div className="flex items-center gap-2 text-sm text-[#86EFAC]" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 border-r border-[#1A1814] flex flex-col overflow-hidden bg-[#0A0A08]">
          ...
          <div className="bg-[#121210] px-5 py-4 border-b border-[#1A1814]">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <Badge variant="neutral" className="border-[#92400E] text-[#F59E0B]">
                ...
                {questionDetail.testCases?.length > 0 && (
                  <div>
                    ...
                    {questionDetail.testCases.map((tc, idx) => (
                      ...
                      >
                        <div className="text-xs font-semibold text-[#F59E0B] mb-3 uppercase tracking-wide" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                          {tc.label || `Test Case ${idx + 1}`}
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <div className="text-[#78716C] text-xs font-medium mb-2 uppercase tracking-wide" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>Input</div>
                            <pre className="bg-[#0A0A08] p-3 text-[#E8E4D9] text-sm overflow-x-auto font-mono border border-[#1A1814]">
                              {tc.stdin}
                            </pre>
                          </div>
                          <div>
                            <div className="text-[#78716C] text-xs font-medium mb-2 uppercase tracking-wide" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                              Expected Output
                            </div>
                            <pre className="bg-[#0A0A08] p-3 text-[#E8E4D9] text-sm overflow-x-auto font-mono border border-[#1A1814]">
                              {tc.expectedOutput}
          ...
          <div className="bg-[#121210] px-5 py-4 border-t border-[#1A1814] flex items-center justify-between">
            ...
            <Button onClick={goToNext} disabled={currentQuestionIndex === questions.length - 1} variant="secondary" size="sm">
              Next
        ...
        <div className="w-1/2 flex flex-col overflow-hidden bg-[#121210]">
          ...
          <div className="bg-[#121210] px-5 py-3 border-b border-[#1A1814] flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              >
                {(questionDetail?.languages || ["python", "javascript", "java", "cpp"]).map(
                  (lang) => (
            ...
            <div className="flex items-center gap-3">
              <Button onClick={handleRun} disabled={isRunning || !code.trim()} variant="secondary" size="sm">
                <Play className={`w-4 h-4 ${isRunning ? "animate-spin" : ""}`} />
                {isRunning ? "Running..." : "Run"}
              </Button>
              <Button onClick={handleSubmitAnswer} disabled={isSubmittingAnswer || !code.trim()} variant="primary" size="sm">
                <Send className="w-4 h-4" />
          ...
          {(runResults || submitResults) && (
            <div className="bg-[#121210] border-t border-[#1A1814] max-h-64 overflow-y-auto">
              <div className="p-4">
                {submitResults && (
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      >
                        {submitResults.verdict?.replace("_", " ").toUpperCase()}
                      </span>
                      <span className="text-[#A29A8C] text-sm" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                        ({submitResults.passed}/{submitResults.total} test cases
                        passed)
                      </span>
                    </div>
                    <div className="text-sm text-[#A29A8C]" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                ...
                {runResults && !submitResults && (
                  <div>
                    <div className="text-sm font-semibold text-[#A29A8C] mb-2" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                      Run Results ({runResults.passedCount}/{runResults.totalCount}{" "}
                      passed)
                    </div>
                      ...
                      >
                        ...
                        {!r.passed && (
                          <div className="text-sm">
                            ...
                            {r.timedOut && (
                              <div className="text-yellow-400">Time Limit Exceeded</div>
                            )}
                            {!r.compileError &&
                              !r.runtimeError &&
                              !r.timedOut && (
                                <div>
                                  <div className="text-[#A29A8C]" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
          <Card className="p-8 max-w-md w-full mx-4 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-[#A29A8C]" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                Are you sure you want to submit? You still have{" "}
                <span className="text-[#E8E4D9] font-bold bg-[#1A1814] border border-[#1A1814] px-2 py-1">{formattedTime}</span>{" "}
                remaining.
              </p>
            </div>

            <div className="mb-6 bg-[#0A0A08] p-4 border border-[#1A1814]">
              <div className="text-sm text-[#A29A8C] mb-3 font-semibold" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                Question Progress:
              </div>
              ...
              <div className="flex justify-center gap-4 mt-4 text-xs text-[#78716C]" style={{ fontFamily: "'Rajdhani', system-ui, sans-serif" }}>
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-green-600 rounded"></span> Solved</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-yellow-600 rounded"></span> Partial</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-[#1A1814] rounded border border-[#1A1814]"></span> Not submitted</span>
              </div>
            </div>
