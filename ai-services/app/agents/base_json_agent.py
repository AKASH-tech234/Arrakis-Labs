...
def run_json_agent(
    *,
    ...
    logger.debug(f"   └─ Cache miss - proceeding with LLM call")
    ...
    try:
        logger.debug(f"   └─ Getting LLM instance...")
        llm = get_llm(temperature=0.2)
        logger.debug(f"   └─ LLM ready")
        ...
        logger.debug(f"   └─ Chain constructed, invoking LLM...")
        ...
        try:
            ...
            logger.info(f"✅ [{agent_name}] LLM call successful on first attempt")

        except OutputParserException as ope:
            logger.warning(f"⚠️ [{agent_name}] Output parsing failed: {str(ope)[:100]}")
            logger.debug(f"   └─ Attempting correction prompt...")
            ...
            try:
                ...
                logger.error(f"❌ [{agent_name}] Correction also failed: {type(e2).__name__}: {e2}")
                record_metric(agent_name, time.time() - start)
                logger.warning(f"⚠️ [{agent_name}] Returning fallback response")
                return fallback

        # -------------------------
        # CACHE WRITE (AFTER SUCCESS)
        # -------------------------
        logger.debug(f"   └─ Writing result to cache...")
        set_cached(cache_key, result.model_dump())
        record_metric(agent_name, time.time() - start)
        logger.info(f"✅ [{agent_name}] COMPLETED in {time.time()-start:.2f}s")
        return result
        
    except Exception as e:
        logger.error(f"❌ [{agent_name}] UNEXPECTED ERROR: {type(e).__name__}: {e}")
