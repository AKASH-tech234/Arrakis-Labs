...
export async function syncPlatformProfile(platformProfileId) {
  ...
  try {
    ...
    if (profile.platform === "leetcode") {
      stats = await fetchLeetCode(profile.handle);
    } else if (profile.platform === "codeforces") {
      stats = await fetchCodeforces(profile.handle);
    } else {
      // For codechef/atcoder/hackerrank/custom: allow manual fallback; store zeroed stats.
      stats = {
        totalSolved: 0,
        totalAttempted: 0,
        last30DaysSolved: 0,
    ...
    // quick skill strength normalization (empty by default)
    const skills = new Map();
    for (const [k, v] of Object.entries(stats.skills || {})) {
      skills.set(k, {
        solved: v.solved || 0,
        attempted: v.attempted || v.solved || 0,
        accuracy: v.attempted ? (v.solved / v.attempted) * 100 : 0,
        strengthLevel: strengthLevel(v.solved || 0),
      });
    }
    ...
    return { skipped: false, platform: profile.platform };
  } catch (err) {
