...
export async function computeAggregatedStats(userId) {
  ...
  // activity / consistency from internal daily series (arrakis)
  const arrakis = platformStats.find((p) => p.platform === "arrakis");
  const daily = (arrakis?.daily || []).slice(-365);
  const activeDays = daily.filter((d) => (d.solved || 0) > 0).length;
  ...
  // weekly/monthly trends from arrakis daily
  const weeklyMap = new Map();
  const monthlyMap = new Map();
  for (const d of daily) {
    const dt = new Date(`${d.date}T00:00:00Z`);
    const wk = weekStartKey(dt);
    weeklyMap.set(wk, (weeklyMap.get(wk) || 0) + (d.solved || 0));
    const mk = monthKey(dt);
    monthlyMap.set(mk, (monthlyMap.get(mk) || 0) + (d.solved || 0));
  }
